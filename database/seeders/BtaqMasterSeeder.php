...
class BtaqMasterSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // 2. Seed tabel_alquran (114 Surah, each with its actual verses)
        $surahs = [
            1 => ["Al-Fatihah", 7],
            2 => ["Al-Baqarah", 286],
            3 => ["Ali 'Imran", 200],
        ...
        $alquranData = [];
        foreach ($surahs as $surahNo => $surahInfo) {
            $surahName = $surahInfo[0];
            $totalAyat = $surahInfo[1];
            for ($ayat = 1; $ayat <= $totalAyat; $ayat++) {
                $alquranData[] = [
                    'surat' => $surahName,
                    'ayat' => $ayat,
                ];
        ...
        // Chunk insert since total records is 6236
        foreach (array_chunk($alquranData, 500) as $chunk) {
            DB::table('tabel_alquran')->insert($chunk);
        }
    }
}
