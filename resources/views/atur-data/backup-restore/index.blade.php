...
@section('title', 'Backup & Restore Database — SmartSchool')
@section('header_title', 'Backup & Restore Database')
@section('header_subtitle', 'Kelola ekspor, impor, dan pemulihan data database SQL')
...
<div class="page-content">
    ...
    <div class="db-backup-layout" style="display: flex; flex-direction: column; gap: 24px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(360px, 1fr)); gap: 24px;">
            ...
            <!-- CARD 1: EKSPOR & BUAT BACKUP -->
            <div class="card">
                <div class="card-header">
                    <h2 class="card-title" style="display: flex; align-items: center; gap: 10px;">
                        <i class="fa-solid fa-database" style="color: var(--primary-color, #3b82f6);"></i>
                        <span>Ekspor Database (Backup SQL)</span>
                    </h2>
                </div>
                <div class="card-body" style="display: flex; flex-direction: column; gap: 16px;">
                    <p style="color: #64748b; font-size: 0.92rem; line-height: 1.6; margin: 0;">
                        Unduh atau simpan snapshot lengkap struktur dan data database SmartSchool V3 dalam format file SQL standar (skema tabel + data).
                    </p>
                    ...
                    <div style="display: flex; flex-wrap: wrap; gap: 12px; margin-top: 8px;">
                        <a href="{{ route('atur-data.backup-restore.export') }}" class="btn btn-primary" style="display: inline-flex; align-items: center; gap: 8px; text-decoration: none;">
                            ...
                            <span>Unduh SQL Direct (Export)</span>
                        </a>
                        ...
                        <form action="{{ route('atur-data.backup-restore.store') }}" method="POST" style="margin: 0;">
                            ...
                            <button type="submit" class="btn btn-secondary" style="display: inline-flex; align-items: center; gap: 8px;">
                                ...
                                <span>Simpan Backup ke Server</span>
                            </button>
                        </form>
                    </div>

                    <div style="background: #eff6ff; border-left: 4px solid #3b82f6; padding: 12px 16px; border-radius: 6px; margin-top: 4px;">
                        <small style="color: #1e40af; font-size: 0.85rem; display: flex; align-items: center; gap: 8px;">
                            <i class="fa-solid fa-circle-info"></i>
                            File ekspor dapat digunakan untuk migrasi database atau pemulihan darurat jika terjadi kendala data.
                        </small>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header">
                    <h2 class="card-title" style="display: flex; align-items: center; gap: 10px;">
                        <i class="fa-solid fa-file-import" style="color: var(--danger-color, #ef4444);"></i>
                ...
                <div class="card-body">
                    <form action="{{ route('atur-data.backup-restore.upload-restore') }}" method="POST" enctype="multipart/form-data" id="form-upload-restore" onsubmit="return confirmRestoreUpload(event)">
                        ...
                        <div class="form-group" style="margin-bottom: 16px;">
                            ...
                            <div class="drop-zone" id="drop-zone" style="border: 2px dashed #cbd5e1; border-radius: 8px; padding: 24px; text-align: center; background: #f8fafc; cursor: pointer; transition: all 0.2s ease;">
                                <i class="fa-solid fa-cloud-arrow-up" style="font-size: 2.2rem; color: #94a3b8; margin-bottom: 8px;"></i>
                                <p style="margin: 0 0 4px 0; font-weight: 500; color: #334155;">Klik atau seret file `.sql` ke area ini</p>
                                <small style="color: #64748b; font-size: 0.82rem;">Format: .sql | Ukuran Maksimal: 100MB</small>
                                <input type="file" name="sql_file" id="sql_file" accept=".sql" style="display: none;" required onchange="showSelectedFileName(this)">
                            </div>
                            <div id="file-name-display" style="margin-top: 8px; font-weight: 600; color: #0284c7; font-size: 0.88rem; display: none;"></div>
                        </div>

                        <div style="background: #fef2f2; border-left: 4px solid #ef4444; padding: 12px 16px; border-radius: 6px; margin-bottom: 16px;">
                            <small style="color: #991b1b; font-size: 0.84rem; display: flex; align-items: flex-start; gap: 8px;">
                                <i class="fa-solid fa-triangle-exclamation" style="margin-top: 2px;"></i>
                                <span><strong>Peringatan:</strong> Proses restore akan menimpa skema dan data database saat ini. Pastikan Anda telah membuat backup data sebelum melakukan restore.</span>
                            </small>
                        </div>
        ...
        <!-- ROW 2: TABEL RIWAYAT BACKUP TERPANTAU DI SERVER -->
        <div class="card">
            ...
            <div class="card-body" style="padding: 0;">
                                ...
                                <td style="padding: 14px 18px; text-align: right;">
                                    <div style="display: inline-flex; gap: 8px;">
                                        ...
                                        <a href="{{ route('atur-data.backup-restore.download-saved', $item['filename']) }}" 
                                           class="btn btn-light btn-sm" title="Unduh File SQL" style="color: #0284c7; border: 1px solid #e2e8f0;">
                                        ...
                                        <form action="{{ route('atur-data.backup-restore.restore-saved', $item['filename']) }}" method="POST" style="margin: 0;" onsubmit="return confirmRestoreSaved('{{ $item['filename'] }}', event)">
                                            @csrf
                                            <button type="submit" class="btn btn-warning btn-sm" title="Restore Database dari File Ini">
                ...
                <div style="padding: 40px; text-align: center; color: #94a3b8;">
                    ...
                    <small>Klik tombol "Simpan Backup ke Server" di atas untuk membuat file backup baru.</small>
                </div>
                @endif
            </div>
        </div>
...
<script>
    ...
    dropZone.addEventListener('drop', (e) => {
        ...
        if (e.dataTransfer.files.length > 0) {
            ...
            showSelectedFileName(fileInput);
        }
    });

    function showSelectedFileName(input) {
        if (input.files && input.files[0]) {
            nameDisplay.innerHTML = '<i class="fa-solid fa-file-code"></i> File Terpilih: ' + input.files[0].name + ' (' + (input.files[0].size / (1024 * 1024)).toFixed(2) + ' MB)';
