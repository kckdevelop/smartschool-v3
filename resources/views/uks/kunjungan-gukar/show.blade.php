...
@section('title', 'Detail Kunjungan UKS Gukar — SmartSchool')
@section('header_title', 'Detail Kunjungan UKS Gukar')
@section('header_subtitle', 'Detail data kunjungan Guru & Karyawan ke Unit Kesehatan Sekolah')

@section('content')
@php
    $isGuru  = !is_null($kunjungan->id_guru);
    $nama    = $isGuru ? ($kunjungan->guru?->nama_guru ?? '-') : ($kunjungan->karyawan?->nama_karyawan ?? '-');
    $noId    = $isGuru ? ($kunjungan->guru?->no_id ?? '-') : ($kunjungan->karyawan?->no_id ?? '-');
    $peran   = $isGuru ? 'Guru' : 'Karyawan';
@endphp
<div class="page-content">
    <div style="margin-bottom:20px;">
        <a href="{{ route('uks.kunjungan-gukar.index') }}" class="btn btn-secondary btn-sm">
        ...
        <div class="visit-hero">
            ...
            <div>
                <div style="margin-bottom:6px;">
                    <span class="badge {{ $isGuru ? 'badge-info' : 'badge-warning' }}" style="font-size:0.82rem;">
                        {{ $peran }}
                    </span>
                </div>
                <h2 class="visit-title">{{ $nama }}</h2>
                <div class="visit-meta">
                    <span><i class="fa-solid fa-id-card"></i> NIP/ID: {{ $noId }}</span>
...
.visit-title { font-size: 1.35rem; font-weight: 800; margin: 4px 0 6px 0; }
.visit-meta { font-size: 0.82rem; opacity: 0.9; display: flex; flex-wrap: wrap; gap: 12px; }
.visit-meta span { display: flex; align-items: center; gap: 6px; background: rgba(255,255,255,0.15); padding: 4px 10px; border-radius: 20px; }
.detail-section-title { font-size: 0.78rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.7px; color: var(--color-primary); margin-bottom: 16px; display: flex; align-items: center; gap: 8px; padding-bottom: 8px; border-bottom: 1.5px solid rgba(13,148,136,0.1); }
.info-grid { display: grid; grid-template-columns: 1.5fr 1fr; gap: 24px; }
.info-item { display: flex; flex-direction: column; gap: 6px; }
.info-label { font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.6px; color: var(--text-muted); }
.empty-state { text-align: center; padding: 24px; background: #f8fafc; border: 1px dashed #e2e8f0; border-radius: 8px; }
@media(max-width:768px) { .info-grid { grid-template-columns: 1fr; } .visit-hero { flex-direction: column; align-items: flex-start; gap: 16px; } }
</style>
@endpush
@endsection
